...
function settingsDialog() {
    ...
    if (typeof(Banana.Ui.openPropertyEditor) !== 'undefined') {
        ...
        param.auto_layout = Banana.Ui.getInt('Settings', texts.param_auto_layout, param.auto_layout);
        if (param.auto_layout === undefined)
            return;
...
function convertParam(param) {
    ...
    var currentParam = {};
    currentParam.name = 'auto_layout';
    currentParam.title = texts.param_auto_layout;
    currentParam.type = 'bool';
    currentParam.value = param.auto_layout ? true : false;
    currentParam.readValue = function() {
        param.auto_layout = this.value;
    }
...
function verifyParam(param) {
    if (!param.auto_layout)
        param.auto_layout = false;
...
function printInvoice(jsonInvoice, repDocObj, param) {
    ...
    // Document title
    var docTitle = getTitle(invoiceObj, texts, param) + ": " + invoiceObj.document_info.number;
    ...
    var invert = 1;
    if (param.auto_layout && invoiceObj.billing_info.total_to_pay * 1 <= 0) invert = -1;
    ...
    **********************/
    ...
    var tableRow = titleTable.addRow();
    tableRow.addCell(getTitle(invoiceObj, texts, param) + " " + invoiceObj.document_info.number, "bold title");
    ...
    for (var i = 0; i < invoiceObj.items.length; i++) {
        ...
        if (className != "note_cell" || (className == "note_cell" && invoiceObj.items.filter(function(pos) { return pos.item_type == 'note' && pos.origin_row == item.origin_row })[0].index == item.index)) {
            ...
            if (className == "note_cell") {
                ...
                tableRow.addCell(toInvoiceAmountFormat(invoiceObj, item.total_amount_vat_inclusive / invert), classNameEvenRow + " padding-right amount " + className, 1);
            }
        } else {
            s++;
        }
    ...
    if (invoiceObj.billing_info.total_rounding_difference) {
        ...
        tableRow.addCell("", "", 1);
        tableRow.addCell(toInvoiceAmountFormat(invoiceObj, invoiceObj.billing_info.total_rounding_difference / invert), "padding-right amount", 1);
    }
    ...
    tableRow.addCell(toInvoiceAmountFormat(invoiceObj, invoiceObj.billing_info.total_to_pay / invert), "padding-right bold amount border-bottom border-top total", 1);
    ...
    for (var i = 0; i < invoiceObj.billing_info.total_vat_rates.length; i++) {
        ...
        vatInfo += " = " + toInvoiceAmountFormat(invoiceObj, invoiceObj.billing_info.total_vat_rates[i].total_vat_amount / invert) + " " + invoiceObj.document_info.currency;
    ...
    if (param.auto_layout && invoiceObj.billing_info.total_to_pay * 1 <= 0) {
...
function getTitle(invoiceObj, texts, param) {
    var documentTitle = (param.auto_layout && invoiceObj.billing_info.total_to_pay * 1 > 0) ? texts.invoice : texts.statement;
...
function printInvoiceDetails(invoiceObj, repDocObj, param, texts, rowNumber) {
    ...
    var cell1 = tableRow.addCell("", "", 1);
    cell1.addParagraph(getTitle(invoiceObj, texts, param) + " " + invoiceObj.document_info.number, "");
    cell1.addParagraph(texts.date + ": " + Banana.Converter.toLocaleDateFormat(invoiceObj.document_info.date), "");
    cell1.addParagraph(texts.customer + ": " + invoiceObj.customer_info.number, "");
    //Payment Terms
    if (invoiceObj.billing_info.payment_term) {
...
function setInvoiceStyle(reportObj, repStyleObj, param) {
    ...
    repStyleObj.addStyle(".addressCol1", "width:53%");
...
function setInvoiceTexts(language) {
    ...
    if (language == 'it') {
        ...
        texts.param_auto_layout = 'Auto layout ' + texts.invoice + '/' + texts.statement + ' (1=yes, 0=no)';
